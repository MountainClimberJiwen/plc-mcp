...
namespace Web
{
    public class PLCProject
    {
        ...
        {
            var dirPath =  _sclCodePath;
            Console.WriteLine($"dirPath: {dirPath}");
            try
            {
                // scl code path end with .scl
                if (!dirPath.EndsWith(".scl"))
                {
                    ...
                    Console.WriteLine($"Importing SCL file {dirPath}");
                    // check if file exists
                    if (!File.Exists(dirPath))
                    {
                        Console.WriteLine($"SCL file not found: {dirPath}");
                        throw new InvalidOperationException($"SCL file not found: {dirPath}");
                    }
                    var name = Path.GetFileName(dirPath) + DateTime.Now.ToString("yyyyMMddHHmmss");
                    var externalSourceService = _plcSoftware.ExternalSourceGroup.ExternalSources.CreateFromFile(name, dirPath);
                    externalSourceService.GenerateBlocksFromSource();
                }
    ...
    public class TiaPortalProject : IDisposable
    {
        ...
        public TiaPortalProject(string plcName, string sclCodePath, AppConfig config)
        {
            ...
            _projectPath = "E:\\tia\\test\\test.ap19";
        ...
        static void Main2(string[] args)
        {
            try
            {
                ...
                // 获取命令行参数
                var relativePath = args.Length > 1 ? args[1] : "scl";
                string sclCodeDirPath = Path.Combine(config.SclCodeRootDir ?? string.Empty, relativePath);
                // 兼容使用'/'或'\\'作为分隔符
                var plcName = sclCodeDirPath.Replace('\\', '/').Split('/').Last();
